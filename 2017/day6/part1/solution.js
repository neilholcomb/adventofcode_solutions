...
those blocks among the banks. To do this, it removes all of the blocks 
from the selected bank, then moves to the next-highest-indexed memory 
...
both of which have three blocks. The first bank wins the tie, and its three blocks are distributed evenly over the other three banks, 
...
Given the initial block counts in your puzzle input, how many redistribution cycles must be completed before 
...
export const solution1 = input => {
  ...
  while (!duplicateFound) {
    ...
    duplicateFound = seen[hash] || false

    seen[hash] = true

    duplicateFound = duplicateFound || steps >= 30000
  }
...
//one liner solution
//prettier-ignore
export const golf = x=>{let seen={[x.join('')]:1},y,h,m,n,l=x.length-1,s=0;while(!y){s++;m=[...x].sort((a,b)=>b-a)[0];n=x.findIndex(i=>i==m);x[n]=0;for(let i=m;i>0;i--){n+=1;n=n>l?0:n;x[n]+=1}h=x.join('');y=seen[h];seen[h]=1}return s}
